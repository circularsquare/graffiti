...
| 4 | [x] | Early-reject in `scanCells` Sutherland-Hodgman (exact tri-AABB vs. cell-AABB test before the 4 clips) | L | L | M | Done 2026-04-19: added `if (poly.length < 3) continue;` between each of the 4 clips. Catches diagonal triangles whose UV-bbox spans a cell but whose triangle shape misses it — first clip empties the poly, remaining 3 clips skipped. No change to correctness (final `poly.length < 3` check was already the gate). |
...
| 20 | [x] | Cap render loop to 60 Hz on high-refresh monitors | L | L | L-M | Done 2026-04-19: `FRAME_INTERVAL = 1000/60 - 0.5` gate at the top of `animate()`. On a 144 Hz monitor this ~2.4× reduces render work. Tried 45 Hz — RAF aliasing on a 60 Hz monitor collapses it to 30 Hz (the cap can only produce divisors of the monitor rate). Reverted to 60. |
